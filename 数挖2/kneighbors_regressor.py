...
def create_regression_data(n):
    ...
    y = np.sin(X).ravel()  #等价于reshape函数
    
    y[::5] += 1 * (0.5 - np.random.rand(int(n/5))) # 每隔 5 个样本就在样本的值上添加噪音
    return train_test_split(X, y,test_size=0.25,random_state=0)# 进行简单拆分，测试集大小占 1/4
...
def test_KNeighborsRegressor_k_w(*data):
    ...
    weights=['uniform','distance']  #观测的权重
...
def test_KNeighborsRegressor_k_p(*data):
    ...
    Ps=[1,2,10]  #距离的定义，1为曼哈顿距离，2为欧氏距离，等等
    ...
    for P in Ps:
        ...
        for K in Ks:
            ...
            regr.fit(X_train,y_train)
            testing_scores.append(regr.score(X_test,y_test))  #R squre
            training_scores.append(regr.score(X_train,y_train))
